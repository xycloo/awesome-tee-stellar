...
use stellar_xdr::next::{
    ...
    Limits as BridgeLimits, Transaction, TransactionEnvelope, TransactionMeta, TransactionPhase,
...
mod retro_snapshot {
    ...
    impl retroshade::soroban_env_host::storage::SnapshotSource for RetroshadeSnapshot {
        ...
        > {
            let xdred = configurable_entry_and_ttl(
                key.as_ref().to_xdr(RetroshadeLimits::none()).unwrap(),
                self.base_url.clone(),
            )
...
impl Executor {
    ...
    fn get_feebumptx_hash(&self, tx: &FeeBumpTransaction) -> [u8; 32] {
    ...
    pub async fn retroshades_main(&self, meta: stellar_xdr::next::LedgerCloseMeta) {
        ...
        let retroshade_effects = self
            .execute_retroshades(&self.retroshade_contracts, meta)
            .await;
    ...
    async fn execute_retroshades<'a>(
        ...
        for contract in mercury_contracts {
            ...
            for (index, contract) in mercury_contracts.iter().enumerate() {
                if let Some(contracts) = &contract.contracts {
                    for id in contracts {
                        ...
                        if let Ok(id) = id {
                            mercury_contracts_hashmap.insert(
                                retroshade::soroban_env_host::xdr::Hash(id.0),
                                codes[index],
                            );
                        }
                    }
            ...
            let ledger_info = {
                ...
                ledger_info.network_id = self.network;
                    ...
                    for transaction_envelope in envelopes {
                        match transaction_envelope {
                            TransactionEnvelope::Tx(v1) => {
                                let tx_hash = self.get_tx_hash(&v1.tx);
                                ...
                                if let Some(associated_meta) = associated_meta {
                                    ...
                                    if let Ok(true) = has_retroshades {
                                        ...
                                        let retroshades = retroshades_execution
                                            ...
                                            .map_err(|e| {
                                                anyhow::anyhow!("retroshades error {:?}", e)
                                            });
                            ...
                            TransactionEnvelope::TxFeeBump(feebump) => {
                                ...
                                let tx_hash = self.get_feebumptx_hash(&inner_tx);
                                let mut associated_meta = None;
                            ...
                            TransactionEnvelope::Tx(v1) => {
                                ...
                                for txmeta in &processing {
                                    if let TransactionMeta::V3(_v3) = &txmeta.tx_apply_processing {
                                        if tx_hash == txmeta.result.transaction_hash.0 {
                                            associated_meta =
                                                Some(txmeta.tx_apply_processing.clone());
                                        }
                                    } else if let TransactionMeta::V4(_v4) =
                                        &txmeta.tx_apply_processing
                                    {
                                        if tx_hash == txmeta.result.transaction_hash.0 {
                                            associated_meta =
                                                Some(txmeta.tx_apply_processing.clone());
